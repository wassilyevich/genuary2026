...
const sketch = ({ context, width, height, units }) => {
  ...
  grid.drawBalls(ballPaths, wallPaths, speedyPaths);
  ...
  return ({ context, width, height, units, exporting }) => {
    ...
    let speedyLines = pathsToPolylines(speedyPaths, { units });
    ...
    if (!exporting) {
      return renderPaths([...frameLines, ...ballLines, ...wallLines, ...speedyLines], {
        context,
        width,
        height,
        units: settings.units,
...
class Grid {
  ...
  drawBalls(ballPaths, wallPaths, speedyPaths) {
    this.allFrames.forEach(frame => {
      frame.drawBall(ballPaths, this.origin, wallPaths, speedyPaths);
    });
  }
...
class Frame {
  ...
  initiateBall() {
    ...
    this.speedyLines = new SpeedyLines();
  ...
  updateFrame(flight, squash) {
    ...
    if (this.index < flight / 2) {
      ...
      this.drawSpeedyLines = true;
      this.speedyLines.direction = 'right';
    }
    ...
    // FLIGHT B/2 (start): k: 1.5 --> 1
      ...
      this.ball.updateEllipse(this.ball.position, 2, k);
      this.drawSpeedyLines = true;
      this.speedyLines.direction = 'left';
    }
  ...
  drawBall(paths, origin, wallPaths, speedyPaths) {
    let frameCorner = {
    ...
    if (this.drawSpeedyLines) {
      this.speedyLines.draw(speedyPaths, frameCorner, this.frameWidth, this.frameHeight, this.ball);
    }
  }
}
...
class SpeedyLines {
  ...
  draw(paths, frameCorner, frameWidth, frameHeight, ball) {

    // Moving to the right
    if (this.direction === 'right') {
      for (let i = 0; i < params.nSpeedyTries; i++) {
        const path = createPath();
        if (Random.chance(params.nSpeedyChance)) {
          const x1 = Random.range(frameCorner.x + frameWidth / 4, frameCorner.x + frameWidth / 2 + ball.a);
          const y1 = Random.range(frameCorner.y + frameHeight / 2 - params.speedyYRange * ball.b, frameCorner.y + frameHeight / 2 + params.speedyYRange * ball.b);
          const x2 = x1 - Random.range(frameWidth / params.speedyXMin, frameWidth / params.speedyXMax);
          let y2 = 0;
          if (y1 >= frameCorner.y + frameHeight / 2) {
            y2 = y1 + Random.range(frameHeight / 100, frameHeight / 50);
          }
          else {
            y2 = y1 - Random.range(frameHeight / 100, frameHeight / 50);
          }
      ...
      for (let i = 0; i < params.nSpeedyTries; i++) {
        const path = createPath();
        if (Random.chance(params.nSpeedyChance)) {
          const x1 = Random.range(frameCorner.x + frameWidth / 2 - ball.a, frameCorner.x + 3 * frameWidth / 4);
          const y1 = Random.range(frameCorner.y + frameHeight / 2 - params.speedyYRange * ball.b, frameCorner.y + frameHeight / 2 + params.speedyYRange * ball.b);
          const x2 = x1 + Random.range(frameWidth / params.speedyXMin, frameWidth / params.speedyXMax);
          let y2 = 0;
          if (y1 >= frameCorner.y + frameHeight / 2) {
