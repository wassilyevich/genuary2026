...
const sketch = async ({ context, width, height, units, exporting, update }) => {
    ...
    const vasil = await load("./inspiration/pliendvc.jpg");
    ...
    const plino = await load("./inspiration/daandvc.jpg");
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            const cell = grid.cells[count];
            const vd = plinoData[4 * count];
            const isPlien = checkVal(vd);
            if (isPlien) {
                cell.drawLetter(plinoPaths, vd, params.textScale, shading);
            }
            count++;
        }
    }
...
class Cell {
    constructor(row, col, cellWidth, cellHeight, origin, id) {
        ...
        this.origin = origin;
        this.frameCorner = {
            x: this.origin.x + this.col * this.cellWidth,
            y: this.origin.y + this.row * this.cellHeight,
        };
        this.cellCenter = {
            x: this.frameCorner.x + this.cellWidth / 2,
            y: this.frameCorner.y + this.cellHeight / 2,
        };
        this.id = id;
    }
    ...
    drawFrame(paths) {
        const p = createPath();
        p.moveTo(
            this.origin.x + this.col * this.cellWidth,
            this.origin.y + this.row * this.cellHeight,
        );
        p.lineTo(
            this.origin.x + this.col * this.cellWidth + this.cellWidth,
            this.origin.y + this.row * this.cellHeight,
        );
        p.lineTo(
            this.origin.x + this.col * this.cellWidth + this.cellWidth,
            this.origin.y + this.row * this.cellHeight + this.cellHeight,
        );
        p.lineTo(
            this.origin.x + this.col * this.cellWidth,
            this.origin.y + this.row * this.cellHeight + this.cellHeight,
    ...
    drawDot(paths, radius) {
        const p = createPath();
        p.arc(
            this.origin.x + this.col * this.cellWidth + this.cellWidth / 2,
            this.origin.y + this.row * this.cellHeight + this.cellHeight / 2,
            radius,
            0,
            2 * Math.PI,
        );
    ...
    drawLetter(paths, value, scale, shading) {
        ...
        if (letter === false) {
            ...
            const p = placeRaw(
                t,
                { x: this.cellCenter.x, y: this.cellCenter.y, scale: scale },
                { mode: "paths" },
            );
            paths.push(...p);
        }
...
function generateShading(shadeLevels) {
    ...
    for (let level = 0; level < levels; level++) {
        let index = Math.round(
            math.lerp(0, allChars.length - 1, level / levels),
        );
